...
public class ReportPdf {
	...
	class TableHeader extends PdfPageEventHelper {
		...
		public void onEndPage(PdfWriter writer, Document document) {
			...
			try {
				...
				ResultSet rs =
				               stmt.executeQuery("SELECT d.device_name, d.imei" +
				                                 " FROM Androspy_Case c, Androspy_Device d" +
				                                 " WHERE c.case_id = " + AndrospyMain.gb_CaseId +
				                                 " AND d.device_id = " + AndrospyMain.gb_DeviceId);
				while (rs.next()) {
					footer[0] =
					            new Phrase("Device:" + rs.getString("device_name") + ", IMEI:" +
					                       rs.getString("imei"));
					break;
				}
				...
				ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_LEFT,
				                           new Phrase(footer[0]), document.left(),
				                           document.bottom(), 0);
				ColumnText.showTextAligned(writer.getDirectContent(), Element.ALIGN_RIGHT,
				                           new Phrase(footer[1]), document.right(),
				                           document.bottom(), 0);
	...
	public void generatePdf(String feature) {
		...
		try {
			PdfWriter.getInstance(document, new FileOutputStream(AndrospyMain.gb_CasePath +
			                                                     "Analysis/" + feature + ".pdf"));
	...
	public String generatePdf(ArrayList<String> featurelist) {
		...
		String name =
		              AndrospyMain.gb_CasePath + "Analysis/Reports/" + "Report_" + gen_date +
		                      ".pdf";
	...
	private void addContent(Document document, String feature) {
		...
		try {
			...
			if (feature == "User Credentials") {
				...
				Paragraph p2 =
				               new Paragraph("Contacted Bluetooth Device Information",
				                             SubHeadingFont);
				addEmptyLine(p2, 1);
	...
	private void CreateTable(Paragraph subpara, String feature) {
		...
		try {
			...
			if (feature == "PRIMARY_ACC") {
				query =
				        "SELECT host AS 'Account Type', username AS 'User Name'," +
				                " password AS 'Password (HASHED)'" + " FROM Androspy_Account" +
				...
				query =
				        "SELECT host AS 'Account Type', username AS 'User Name'," +
				                " password AS 'Password'" + " FROM Androspy_Account" +
				                " WHERE case_id = " + AndrospyMain.gb_CaseId + " AND device_id = " +
				                AndrospyMain.gb_DeviceId + " AND primary_acc != " + 1;

			} else if (feature == "SMS") {
				query =
				        "SELECT subject AS 'Subject', body AS 'Message Body'," +
				                " type AS 'Message Type', date_received AS 'Received Date', date_sent AS 'Send Date'" +
				                " FROM Sms" + " WHERE case_id = " + AndrospyMain.gb_CaseId +
				                " AND device_id = " + AndrospyMain.gb_DeviceId;

			} else if (feature == "Phone Book") {
				query =
				        "SELECT contact_no AS 'Contact No', name AS 'Name'," +
				...
				query =
				        "SELECT fromaddress AS 'From Address'," +
				...
				query =
				        "SELECT title AS 'Title', description AS 'Event', location AS 'Location'," +
				...
				query =
				        "SELECT query AS 'Search' , search_date AS 'Time Stamp'" +
				...
				query =
				        "SELECT IFNULL(sender, '') AS 'Sender'," +
				...
				query =
				        "SELECT IFNULL(first_name, '') AS 'First Name'," +
				...
				query =
				        "SELECT IFNULL(notification, '') AS 'Notification'," +
				...
				query =
				        "SELECT first_name AS 'First Name', last_name AS 'Last Name', display_name AS 'Display Name'," +
				...
				query =
				        "SELECT IFNULL(from_display_name, '') AS 'Sender Name'," +
				...
				query =
				        "SELECT from_display_name AS 'Remote Party', from_title AS 'Remote Party Title'," +
				                " message_date AS 'Message Date', subject AS Subject, message_body AS Message" +
				                " FROM Linkedin_Message" +
				                " WHERE case_id = " +
				                AndrospyMain.gb_CaseId +
				                " AND device_id = " +
				                AndrospyMain.gb_DeviceId;
				...
				query =
				        "SELECT title AS Title, url AS URL, " +
				...
				query =
				        "SELECT url AS 'URL', name AS 'Parameter Name', value AS 'Parameter Value'" +
				...
				query =
				        "SELECT search_text AS 'Search Text', " + "search_date AS 'Search Date'" +
				                " FROM WebSearch" + " WHERE case_id = " + AndrospyMain.gb_CaseId +
				                " AND device_id = " + AndrospyMain.gb_DeviceId +
				...
				query =
				        "SELECT name AS Name, expire_month AS 'Expire Month', expire_year AS 'Exire Year', " +
				                "modified_date AS 'Modified Date'" +
				                " FROM Browser_Credit_Card" +
				                " WHERE case_id = " +
				                AndrospyMain.gb_CaseId +
				                " AND device_id = " +
				                AndrospyMain.gb_DeviceId + " AND browser = 'CHROME'";
				...
				query =
				        "SELECT contact_no AS 'Contact No', name AS 'Name'," +
				                ...
				                " FROM Event_Log" + " WHERE case_id = " + AndrospyMain.gb_CaseId +
				                " AND device_id = " + AndrospyMain.gb_DeviceId + " AND logtype = " +
				                100;
				...
				query =
				        "SELECT ssid AS 'SSID', password AS 'Password', key_mgmt AS 'Key Management'" +
				                " FROM Wifi_Info" +
				                " WHERE case_id = " +
				                AndrospyMain.gb_CaseId +
				                " AND device_id = " + AndrospyMain.gb_DeviceId;
				...
				query =
				        "SELECT device_address AS 'Target Device BT Address', mimetype AS 'MIME Type', " +
				...
				query =
				        "SELECT bmac AS 'Bluetooth Address'," +
				...
				query =
				        "SELECT skypename, fullname, birthday, country, city reg_date, gender" +
				                " FROM Skype_Account" + " WHERE case_id = " +
				                AndrospyMain.gb_CaseId + " AND device_id = " +
				                AndrospyMain.gb_DeviceId;
				...
				query =
				        "SELECT skypename, fullname, birthday, displayname, country, city, gender, phone" +
				                " FROM Skype_Contacts" +
				                " WHERE case_id = " +
				                AndrospyMain.gb_CaseId +
				                " AND device_id = " +
				...
				query =
				        "SELECT identity, display_name, call_duration, start_time_stamp" +
				                " FROM Skype_Call" + " WHERE case_id = " + AndrospyMain.gb_CaseId +
				                " AND device_id = " + AndrospyMain.gb_DeviceId;

			} else if (feature == "Skype Messages") {
				System.out.println("Skype Messages");
				query =
				        "SELECT author, from_display_name, dialog_partner, message, time_stamp" +
				                " FROM Skype_Message" + " WHERE case_id = " +
				                AndrospyMain.gb_CaseId + " AND device_id = " +
				                AndrospyMain.gb_DeviceId;
	...
	private void addTitlePage(Document document) {
		...
		try {
			...
			Statement stmt = con.createStatement();
			ResultSet rs =
			               stmt.executeQuery("SELECT first_name, last_name FROM Androspy_User WHERE user_name = '" +
	...
	private void createTableRowFormat(Paragraph preface, String feature) {
		try {
			...
			if (feature == "CaseInfo") {
				query =
				        "SELECT case_id AS 'Case ID', case_name AS 'Case Name', investigator_id AS 'Investigatort', IFNULL(analyst_id, '') as 'Analyst' " +
				                "FROM Androspy_Case " + "WHERE case_id = " + AndrospyMain.gb_CaseId;
			} else if (feature == "DeviceBasicInfo") {
				query =
				        "SELECT IFNULL(device_name, '') AS 'Device Name', IFNULL(model, '') AS 'Model', IFNULL(manufacturer, '') AS 'Manufacturer', IFNULL(os, '') AS OS, IFNULL(build_number, '') AS 'Build Number', IFNULL(sw_number, '') AS 'Software Number', IFNULL(serial_no,'') As 'Serial No'" +
