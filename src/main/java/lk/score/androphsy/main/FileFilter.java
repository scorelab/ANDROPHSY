...
public class FileFilter extends Composite {
	...
	public FileFilter(Composite parent, int style) {
		...
		lblApplicationFileFilter.setLayoutData(new GridData(SWT.CENTER, SWT.FILL, false, false, 1,
		                                                    1));
		...
		btnFilterApplicationFiles.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false,
		                                                     1, 1));
	...
	protected void Filter_Dispatch() {
		try {
			...
			Process proc =
			               Runtime.getRuntime()
			                      .exec("md5deep -r " + AndrospyMain.gb_CasePath +
