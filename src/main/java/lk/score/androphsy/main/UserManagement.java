...
public class UserManagement extends Composite {
	...
	public UserManagement(Composite parent, int style) {
			...
			public void mouseUp(MouseEvent arg0) {
				...
				try {
					...
					if (bnewUser) {
						...
						String pwdigest =
						                  new AdbExecCmd().getPasswordEncrption(txtPassword.getText());
						query =
						        "INSERT INTO Androspy_User (first_name, last_name, nic, " +
						                "user_name, password, user_role) Values ('" +
						                txtFirstName.getText() + "'," + "'" +
						                txtLastName.getText() +
						                "'," +
						                // date + "," +
						                "'" + txtEmail.getText() + "'," + "'" +
						                txtUserName.getText() + "'," + "'" + pwdigest + "'," + "'" +
						                db_role + "'" + ");";

						int c = stmt.executeUpdate(query);
						if (c > 0) {
							String stime = new AdbExecCmd().getCurrenttime();
							AndrospyLog.Logdata(stime, "Added New User " + txtUserName.getText() +
							                           " role " + db_role);
						}
						clearFields();
						...
						query =
						        "UPDATE Androspy_User SET user_role = '" + cmbUserRole.getText() +
						                "'";
						query = query + "WHERE user_id = '" + txtUserId.getText() + "'";
						System.out.println("Update Query " + query);
						stmt.executeUpdate(query);
						String stime = new AdbExecCmd().getCurrenttime();
						AndrospyLog.Logdata(stime, "Update user role " + txtUserName +
						                           " new role " + cmbUserRole.getText());
		...
		btnSearch.setToolTipText("lk.score.androphsy.main.Search user by E-Mail, Contact, Username");
		...
		btnDelete.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent arg0) {
				try {
					...
					Statement stmt = conn.createStatement();
					int count =
					            stmt.executeUpdate("DELETE FROM Androspy_User where user_id = '" +
					                               txtUserId.getText() + "'");
					...
					if (count == 1) {
						...
						msg.setMessage("Unable to deleted user: " + user_name +
