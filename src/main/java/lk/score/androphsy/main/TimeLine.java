...
public class TimeLine extends Composite {
	...
	public TimeLine(Composite parent, int style) {
		...
		btnBeforeDate.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, false, false, 2, 1));
		btnBeforeDate.setText("Before Date");

		btnAfterDate = new Button(this, SWT.CHECK);
		btnAfterDate.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, false, false, 2, 1));
		btnAfterDate.setText("After Date");
		new Label(this, SWT.NONE);

		dateTimeFrom = new DateTime(this, SWT.DROP_DOWN | SWT.BORDER);
		dateTimeFrom.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));

		timeFrom = new DateTime(this, SWT.BORDER | SWT.TIME | SWT.LONG);

		dateTimeTo = new DateTime(this, SWT.DROP_DOWN | SWT.BORDER);
		dateTimeTo.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		...
		btnFilter.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent arg0) {
				...
				String sfromdate =
				                   dateTimeFrom.getYear() + "/" + (dateTimeFrom.getMonth() + 1) +
				                           "/" + dateTimeFrom.getDay();
				...
				String stodate =
				                 dateTimeTo.getYear() + "/" + (dateTimeTo.getMonth() + 1) + "/" +
				                         dateTimeTo.getDay();
				...
				createTimeLine(bBefore, bAfter, sfromdate + " " + sfromtime, stodate + " " +
				                                                             stotime);
			}
		});
		btnFilter.setLayoutData(new GridData(SWT.FILL, SWT.FILL, false, false, 1, 1));
		...
		tbtWebsearch.setText("Default Browser Web lk.score.androphsy.main.Search");
		...
		tbtChromeWS.setText("Chrome Web lk.score.androphsy.main.Search");
	...
	private void createTimeLine(boolean bBefore, boolean bAfter, String sfromdate, String stodate) {
		timelinedummy.listAllEvents("ALL", bBefore, bAfter, sfromdate, stodate);
		timelineevent.listEvents("EVENTLOG", bBefore, bAfter, sfromdate, stodate);
		...
		timelinebrowserhist.listEvents("DEFAULTBROWSERHISTORY", bBefore, bAfter, sfromdate, stodate);
		timelinebookmark.listEvents("DEFAULTBOOKMARK", bBefore, bAfter, sfromdate, stodate);
		timelinewebsearch.listEvents("DEFAULTWEBSEARCH", bBefore, bAfter, sfromdate, stodate);
