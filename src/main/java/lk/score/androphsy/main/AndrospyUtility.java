...
public class AndrospyUtility {
	...
	protected void readRecords(String name) {
		...
		if (name.equals("DEFAULT_BROWSER_HISTORY")) {
			...
			query =
			        "SELECT _id, IFNULL(title, '') AS title, IFNULL(url, '') AS url, "
			...
			appPath += "com.android.browser/databases/browser2.db";
			query =
			        "SELECT _id, IFNULL(search, '') AS search, IFNULL(date,'') AS 'date'"
			                + " FROM searches";
			...
			query =
			        "SELECT datetime((creation_utc/1000000 -11644473600), \"unixepoch\", \"localtime\") AS creation_utc, IFNULL(host_key,'') AS host_key,"
			...
			// form data
			query =
			        "SELECT _id, IFNULL(urlid,'') AS urlid, IFNULL(name,'') AS name, IFNULL(value,'') AS value"
			                + " FROM formdata";
			...
			query =
			        "SELECT v.id, IFNULL(u.title, '') AS title, IFNULL(u.url, '') AS url,"
			...
			appPath += "com.android.chrome/app_chrome/Default/History";
			query =
			        "SELECT IFNULL(k.url_id,'') AS url_id, IFNULL(k.term, '') AS term,"
			...
			query =
			        "SELECT guid, IFNULL(name_on_card, '') AS name_on_card, "
			...
			query =
			        "SELECT name, value, host_key, path, expires_utc, last_access_utc, creation_utc, "
			...
			query =
			        "SELECT _id, IFNULL(first_name, '') AS first_name, IFNULL(last_name, '') AS last_name,"
			...
			appPath += "com.linkedin.android/databases/linkedin.db";
			query =
			        "SELECT _id, IFNULL(from_display_name, '') AS from_display_name, "
			...
			appPath += "com.linkedin.android/databases/linkedin.db";
			query =
			        "SELECT _id, IFNULL(person, '') AS person, IFNULL(connections, '') AS connections"
			                + " FROM profile";
			...
			query =
			        "SELECT _id, IFNULL(number, '') AS number, IFNULL(date, '') AS 'date', "
			...
			query =
			        "select name_raw_contact_id, IFNULL(times_contacted, '') AS times_contacted, "
			                + "IFNULL(last_time_contacted, '') AS last_time_contacted, IFNULL(d.data1, '') AS contact_no,"
			                + " IFNULL(x.data1, '') AS name " + "from contacts, data d, data x "
			                + "where name_raw_contact_id = d.raw_contact_id "
			                + "AND d.raw_contact_id = x.raw_contact_id " + "AND d.mimetype_id = 5 "
			                + "AND x.mimetype_id = 7";
			insertRecords("Contacts", appPath, query);
		} else if (name.equals("Gmail")) {
			String gmail = new AdbExecCmd().getPrimaryMail();
			appPath += "com.google.android.gm/databases/mailstore." + gmail + ".db";
			System.out.println("Gmail Path " + appPath);
			File tmp = new File(appPath);
			if (tmp.exists()) {
				System.out.println("Gmail Exists");
				query =
				        "select c._id, IFNULL(m.fromAddress, '') AS fromAddress, IFNULL(m.toAddresses, '') AS toAddresses,"
			...
			query =
			        "SELECT IFNULL(data1, '')AS data1, IFNULL(latitude, '') AS latitude, IFNULL(longitude, '') AS longitude,"
			                + " IFNULL(timestamp, '') AS timestamp" + " FROM suggestions";
			insertRecords("Map", appPath, query);

		} else if (name.equals("Calendar")) {
			appPath += "com.android.providers.calendar/databases/calendar.db";
			query =
			        "SELECT _id, IFNULL(title, '') AS title, IFNULL(eventlocation, '') AS eventlocation,"
			...
			query =
			        "SELECT _id, IFNULL(query, '') AS query, IFNULL(date, '') AS 'date'"
			                + " FROM suggestions";
			...
			appPath += "com.android.browser/databases/webview.db";
			query =
			        "SELECT _id, IFNULL(host, '') AS host, IFNULL(username, '') AS username, IFNULL(password, '') AS password"
			                + " FROM password";
			insertRecords("Accounts", appPath, query);

			String path = AndrospyMain.gb_CasePath + "Acquire/data/system/users/0/accounts.db";
			query =
			        "SELECT _id, IFNULL(type, '') AS type, IFNULL(name, '') AS name, IFNULL(password, '') AS password"
			                + " FROM accounts";
			insertRecords("AccountPrimary", path, query);
		} else if (name.equals("Bluetooth")) {
			appPath += "com.android.bluetooth/databases/btopp.db";
			query =
			        "SELECT _id, IFNULL(mimetype, '') AS mimetype, IFNULL(hint, '') AS hint, IFNULL(_data, '') AS _data,"
			...
			query =
			        "SELECT _id, IFNULL(address,'') AS address, IFNULL(date,'') AS 'date', IFNULL(date_sent, '') AS 'date_sent',"
			...
			query =
			        "SELECT internal_id, IFNULL(first_name,'') AS first_name, IFNULL(last_name,'') AS last_name, IFNULL(display_name,'') AS display_name,"
			...
			query =
			        "SELECT msg_id, IFNULL(text,'') AS text, IFNULL(sender,'') AS sender, IFNULL(timestamp_ms,'') AS timestamp_ms,"
			                + " IFNULL(coordinates,'') AS coordinates"
			                + " FROM messages"
			                + " group by thread_id order by msg_id";
			...
			query =
			        "SELECT _id, IFNULL(updated,'') AS updated, IFNULL(graphql_text_with_entities,'') AS graphql_text_with_entities"
			...
			while (it.hasNext()) {
				...
				query =
				        "SELECT id, skypename, IFNULL(fullname,'') AS fullname, IFNULL(birthday, '') AS birthday,"
			...
			while (it.hasNext()) {
				...
				query =
				        "SELECT id, identity, IFNULL(dispname, '') AS displayname, IFNULL(call_duration,'') AS call_duration,"
			...
			while (it.hasNext()) {
				...
				query =
				        "SELECT id, author, IFNULL(from_dispname,'') AS from_dispname, IFNULL( dialog_partner, '') AS  dialog_partner,"
		...
		try {
			...
			if (identifier == "DEFAULT_BROWSER_HISTORY") {
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Browser_History VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + processSpecialCharacter(rs.getString("title")) +
					                  "', '" + processSpecialCharacter(rs.getString("url")) +
					                  "', '" + formatdate + "', " + rs.getInt("visits") + ", " + 0 +
					                  ", 'DEFAULTB' )";
					int count = mysqlsmt.executeUpdate(sinsert);
					if (count > 0) {
						System.out.println("Added " + count + "Rows to " + identifier);
					} else {
						System.out.println("Insert into " + identifier + " failed");
					}
				}
				...
				while (rs.next()) {
					...
					String formatdate = sf.format(dlastvisit);
					sinsert =
					          "INSERT INTO WebSearch VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + processSpecialCharacter(rs.getString("search")) +
					                  "', '" + formatdate + "', " + 0 + ", 'DEFAULTB' )";
					int count = mysqlsmt.executeUpdate(sinsert);
					if (count > 0) {
						System.out.println("Added " + count + "Rows to " + identifier);
					} else {
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Androspy_Bookmark VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + processSpecialCharacter(rs.getString("title")) +
					                  "', '" + processSpecialCharacter(rs.getString("url")) +
					                  "', " + rs.getInt("deleted") + ", '" +
					                  rs.getString("account_name") + "', '" +
					                  rs.getString("account_type") + "', '" + createddate + "', '" +
					                  modifieddate + "', " + 0 + " )";
				...
				while (rs.next()) {
					...
					Date tmp =
					           new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rs.getString("creation_utc"));
					String sdate = sf.format(tmp);

					tmp =
					      new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rs.getString("expires_utc"));
					String sexpire = sf.format(tmp);

					tmp =
					      new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rs.getString("last_access_utc"));
					String slast = sf.format(tmp);

					sinsert =
					          "INSERT INTO Browser_Cookies (case_id, device_id, created_date, host, "
				...
				while (rs.next()) {
					sinsert =
					          "INSERT INTO WebFormData VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", " + rs.getInt("urlid") + ", '" +
					                  processSpecialCharacter(rs.getString("name")) + "', '" +
					                  processSpecialCharacter(rs.getString("value")) + "', " + 0 +
					                  " )";
					int count = mysqlsmt.executeUpdate(sinsert);
					if (count > 0) {
						System.out.println("Added " + count + "Rows to " + identifier);
				...
				while (rs.next()) {
					sinsert =
					          "INSERT INTO WebForm VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + processSpecialCharacter(rs.getString("url")) + "' " +
					                  ")";
					int count = mysqlsmt.executeUpdate(sinsert);
				...
				while (rs.next()) {
					SimpleDateFormat sf = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");
					Date tmp =
					           new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rs.getString("visit_time"));

					String sdate = sf.format(tmp);

					sinsert =
					          "INSERT INTO Browser_History VALUES (" + rs.getInt("id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + processSpecialCharacter(rs.getString("title")) +
					                  "', '" + processSpecialCharacter(rs.getString("url")) +
					                  "', '" + sdate + "', " + rs.getInt("visit_count") + ", " + 0 +
					                  ", 'CHROME' )";
				...
				while (rs.next()) {
					SimpleDateFormat sf = new SimpleDateFormat("MM/dd/yyyy HH:mm:ss");
					Date tmp =
					           new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rs.getString("last_visit_time"));
					String sdate = sf.format(tmp);

					sinsert =
					          "INSERT INTO WebSearch VALUES (" + rs.getInt("url_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
				...
				while (rs.next()) {
					...
					Date tmp =
					           new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(rs.getString("date_modified"));
					String sdate = sf.format(tmp);

					sinsert =
					          "INSERT INTO Browser_Credit_Card VALUES (" + rs.getInt("guid") +
					                  ", " + AndrospyMain.gb_CaseId + ", " +
					                  AndrospyMain.gb_DeviceId + ", '" +
					                  processSpecialCharacter(rs.getString("name_on_card")) +
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Androspy_Bookmark (id, case_id, device_id, title, url, deleted, created_date, modified_date, favourite, browser)" +
					                  " VALUES (" +
					                  rs.getInt("_id") +
					                  ", " +
					                  AndrospyMain.gb_CaseId +
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Skype_Account  VALUES (" + rs.getInt("id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("skypename") + "', '" +
					                  rs.getString("fullname") + "', '" + rs.getString("birthday") +
					                  "', '" + rs.getString("country") + "', '" +
					                  rs.getString("city") + "', '" + formatdate + "', '" +
					                  rs.getString("gender") + "' " + ")";
				...
				while (rs.next()) {
					sinsert =
					          "INSERT INTO Skype_Contacts  VALUES (" + rs.getInt("id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("skypename") + "', '" +
					                  rs.getString("fullname") + "', '" + rs.getString("birthday") +
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Skype_Message VALUES (" + rs.getInt("id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("author") + "', '" +
					                  rs.getString("from_dispname") + "', '" +
					                  rs.getString("dialog_partner") + "', '" + formatdate +
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Skype_Call  VALUES (" + rs.getInt("id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("identity") + "', '" +
					                  rs.getString("displayname") + "', '" +
					                  rs.getString("call_duration") + "', '" + formatdate + "' " +
					                  ")";
				...
				while (rs.next()) {
					sinsert =
					          "INSERT INTO Linkedin_Connections  VALUES (" + rs.getInt("_id") +
					                  ", " + AndrospyMain.gb_CaseId + ", " +
					                  AndrospyMain.gb_DeviceId + ", '" +
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Linkedin_Invitation VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("from_display_name") + "', '" +
					                  processSpecialCharacter(rs.getString("from_headline")) +
					                  "', '" + formatdate + "', " + 0 + ")";
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Linkedin_Message  VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("from_display_name") + "', '" +
					                  rs.getString("from_headline") + "', '" + formatdate + "', '" +
					                  processSpecialCharacter(rs.getString("subject")) + "', '" +
					                  processSpecialCharacter(rs.getString("body")) + "', " + 0 +
					                  ")";
				...
				while (rs.next()) {
					...
					try {
						...
						sinsert =
						          "INSERT INTO Linkedin_Profile  VALUES (" + rs.getInt("_id") +
						                  ", " + AndrospyMain.gb_CaseId + ", " +
						                  AndrospyMain.gb_DeviceId + ", '" + firtname + "', '" +
						                  lastname + "', '" + displayname + "', '" + formatdate +
						                  "', '" + location + "', " + conn_count + ", " + 0 + ")";
			...
			if (identifier == "Accounts") {
				while (rs.next()) {
					sinsert =
					          "INSERT INTO Androspy_Account VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Contacts VALUES (" + rs.getInt("name_raw_contact_id") +
					                  ", " + AndrospyMain.gb_CaseId + ", " +
					                  AndrospyMain.gb_DeviceId + ", '" +
					                  rs.getString("contact_no") + "', '" + rs.getString("name") +
					                  "', " + rs.getInt("times_contacted") + ", '" + formatdate +
					                  "', " + 0 + ")";
					int count = mysqlsmt.executeUpdate(sinsert);
					if (count > 0) {
						System.out.println("Added " + count + "Rows to " + identifier);
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Sms VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("address") + "', '" + sdatein + "', '" +
					                  sdatesent + "', '" +
					                  processSpecialCharacter(rs.getString("subject")) + "', '" +
					                  processSpecialCharacter(rs.getString("body")) + "', '" +
					                  rs.getString("service_center") + "', " + rs.getInt("type") +
					                  ", " + 0 + ")";
					int count = mysqlsmt.executeUpdate(sinsert);
					if (count > 0) {
						System.out.println("Added " + count + "Rows to " + identifier);
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Gmail_Store (case_id, device_id, fromaddress, toaddress, ccaddress, bccaddress, replytoaddress, "
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Calendar_Event VALUES(" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("title") + "', '" +
					                  rs.getString("description") + "', '" +
					                  rs.getString("eventLocation") + "', '" + sdstart + "', '" +
					                  sdend + "', '" + rs.getString("organizer") + "', " +
					                  rs.getInt("deleted") + ", " + 0 + ")";
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Youtube_History VALUES(" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("query") + "', '" + sdstart + "', " + 0 +
					                  ")";
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Map_Location (case_id, device_id, place, latitude, longitude, timestamp) VALUES(" +
					                  AndrospyMain.gb_CaseId +
					                  ", " +
					                  AndrospyMain.gb_DeviceId +
					                  ", '" +
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Fb_Contacts VALUES (" + rs.getInt("internal_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("first_name") + "', '" +
					                  rs.getString("last_name") + "', '" +
					                  rs.getString("display_name") + "', '" + formatdate + "', '" +
					                  rs.getString("type") + "', " + 0 + ")";
				...
				while (rs.next()) {
					...
					try {
						...
						sinsert =
						          "INSERT INTO Facebook_Message VALUES (" + rs.getInt("msg_id") +
						                  ", " + AndrospyMain.gb_CaseId + ", " +
						                  AndrospyMain.gb_DeviceId + ", '" + name + "', '" +
						                  processSpecialCharacter(rs.getString("text")) + "', '" +
						                  formatdate + "', '" + longitude + "', '" + latitude +
						                  "', " + 0 + ")";
				...
				while (rs.next()) {
					...
					try {
						String notifi_json = rs.getString("graphql_text_with_entities");
						JSONObject jobject = new JSONObject(notifi_json);
						String notification = jobject.getString("text");

						sinsert =
						          "INSERT INTO Facebook_Notifications VALUES (" + rs.getInt("_id") +
						                  ", " + AndrospyMain.gb_CaseId + ", " +
						                  AndrospyMain.gb_DeviceId + ", '" +
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Event_Log VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("number") + "', '" +
					                  rs.getString("name") + "', '" + createddate + "', " +
					                  rs.getInt("duration") + ", " + rs.getInt("type") + ", " +
					                  rs.getInt("logtype") + ", " + rs.getInt("messageid") + ", '" +
				...
				while (rs.next()) {
					...
					sinsert =
					          "INSERT INTO Bluetooth_Tranfer VALUES (" + rs.getInt("_id") + ", " +
					                  AndrospyMain.gb_CaseId + ", " + AndrospyMain.gb_DeviceId +
					                  ", '" + rs.getString("mimetype") + "', '" +
					                  rs.getString("hint") + "', '" + rs.getString("_data") +
					                  "', " + rs.getInt("direction") + ", '" +
					                  rs.getString("destination") + "'," + rs.getInt("visibility") +
					                  ", " + rs.getInt("confirm") + ", " + rs.getString("status") +
					                  ", '" + rs.getString("total_bytes") + "', '" + createddate +
					                  "', " + rs.getString("scanned") + ", " + 0 + " )";
					int count = mysqlsmt.executeUpdate(sinsert);
					if (count > 0) {
						System.out.println("Added " + count + "Rows to " + identifier);
					} else {
