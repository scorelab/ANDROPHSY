...
public class TimeLineDummy extends Composite {
	...
	public void listAllEvents(String feature, boolean bBefore, boolean bAfter, String sfromdate,
	                          String stodate) {
		// -----------
		String query_all =
		                   "SELECT 'Calendar Event' AS EventType, c.title AS 'Remote Party Contact', c.start_time AS tldate" +
		...
		query_all +=
		             "SELECT 'Gmail Inbox' AS EventType, g.toaddress AS 'Remote Party Contact', g.datereceive AS tldate" +
		                     " FROM Gmail_Store g" +
		                     " WHERE g.case_id = " +
		                     AndrospyMain.gb_CaseId +
		                     " AND g.device_id = " +
		...
		query_all +=
		             "SELECT 'Youtube' AS EventType, y.query AS 'Remote Party Contact', y.search_date AS tldate" +
		...
		query_all =
		            "SELECT 'Facebook Contact' AS EventType, fc.display_name AS 'Remote Party Contact', fc.added_time AS tldate" +
		                    " FROM Fb_Contacts fc" +
		                    " WHERE fc.case_id = " +
		                    AndrospyMain.gb_CaseId +
		                    " AND fc.device_id = " +
		...
		query_all =
		            "SELECT 'Facebook Message' AS EventType, fm.sender AS 'Remote Party Contact', fm.conversation_date AS tldate" +
		...
		query_all =
		            "SELECT 'Facebook Notification' AS EventType, fn.notification AS 'Remote Party Contact', fn.update_date AS tldate" +
		...
		query_all =
		            "SELECT 'Linkedin Profile' AS EventType, lp.display_name AS 'Remote Party Contact', lp.join_date AS tldate" +
		...
		query_all =
		            "SELECT 'Linkedin Invitations' AS EventType, li.from_display_name AS 'Remote Party Contact', li.invitation_date AS tldate" +
		...
		query_all =
		            "SELECT 'Linkedin Messages' AS EventType, lm.from_display_name AS 'Remote Party Contact', lm.message_date AS tldate" +
		...
		query_all +=
		             "SELECT CASE WHEN (browser = 'DEFAULTB') THEN 'Default Browser Bookmark'" +
		                     "				  WHEN (browser = 'CHROME') THEN 'Chrome Bookmark' END AS EventType, b.url AS 'Remote Party Contact', b.modified_date AS tldate" +
		                     " FROM Androspy_Bookmark b" + " WHERE b.case_id = " +
		                     AndrospyMain.gb_CaseId + " AND b.device_id = " +
		                     AndrospyMain.gb_DeviceId;
		...
		query_all +=
		             "SELECT CASE WHEN (browser = 'DEFAULTB') THEN 'Default Browser History'" +
		                     "				  WHEN (browser = 'CHROME') THEN 'Chrome History' END AS EventType, bh.url AS 'Remote Party Contact', bh.last_visit_date AS tldate" +
		                     " FROM Browser_History bh" + " WHERE bh.case_id = " +
		                     AndrospyMain.gb_CaseId + " AND bh.device_id = " +
		                     AndrospyMain.gb_DeviceId;
		...
		query_all +=
		             "SELECT CASE WHEN (browser = 'DEFAULTB') THEN 'Default Browser Websearch'" +
		                     ...
		                     " FROM WebSearch ws" + " WHERE ws.case_id = " +
		                     AndrospyMain.gb_CaseId + " AND ws.device_id = " +
		                     AndrospyMain.gb_DeviceId;
		...
		query_all +=
		             "SELECT CASE WHEN (bcc.browser = 'DEFAULTB') THEN 'Default Browser Websearch'" +
		...
		query_all +=
		             "SELECT 'SMS' AS EventType, s.address AS 'Remote Party Contact', s.date_received AS 'TimeStamp'" +
		                     " FROM Sms s" +
		                     " WHERE s.case_id = " +
		                     AndrospyMain.gb_CaseId +
		                     " AND s.device_id = " + AndrospyMain.gb_DeviceId;
		if (bBefore && !bAfter) {
			query_all +=
		...
		if (bBefore && !bAfter) {
			query_all +=
			             " AND STR_TO_DATE( el.date, '%m/%d/%Y %T') <= STR_TO_DATE('" + sfromdate +
			                     "', '%Y/%m/%d %T')";
		} else if (!bBefore && bAfter) {
			query_all +=
			             " AND STR_TO_DATE( el.date, '%m/%d/%Y %T') >= STR_TO_DATE('" + stodate +
			                     "', '%Y/%m/%d %T')";
		} else if (!bBefore && !bAfter) {
			query_all +=
			             " AND STR_TO_DATE( el.date, '%m/%d/%Y %T') >= STR_TO_DATE('" + sfromdate +
	...
	public void listEvents(String feature, boolean bBefore, boolean bAfter, String sfromdate,
	                       String stodate) {
		...
		String query_sms =
		                   "SELECT 'SMS' AS EventType, s.address AS 'Remote Party Contact', s.date_received AS tldate" +
		                           " FROM Sms s" +
		                           " WHERE s.case_id = " +
		                           AndrospyMain.gb_CaseId +
		                           " AND s.device_id = " + AndrospyMain.gb_DeviceId;
		...
		String query_fbnotification =
		                              "SELECT 'Facebook Notification' AS EventType, fn.notification AS 'Remote Party Contact', fn.update_date AS tldate" +
		                                      " FROM Facebook_Notifications fn" +
		                                      " WHERE fn.case_id = " +
		                                      AndrospyMain.gb_CaseId +
		                                      " AND fn.device_id = " + AndrospyMain.gb_DeviceId;

		String query_lnprofile =
		                         "SELECT 'Linkedin Profile' AS EventType, lp.display_name AS 'Remote Party Contact', lp.join_date AS tldate" +
		                                 " FROM Linkedin_Profile lp" +
		                                 " WHERE lp.case_id = " +
		                                 AndrospyMain.gb_CaseId +
		...
		String query_Def_BrowserHist =
		                               "SELECT 'Default Browser Browser History' AS EventType, bh.url AS 'Remote Party Contact', bh.last_visit_date AS tldate" +
		                                       " FROM Browser_History bh" +
		                                       " WHERE bh.case_id = " +
		                                       AndrospyMain.gb_CaseId +
		                                       " AND bh.device_id = " +
		                                       AndrospyMain.gb_DeviceId +
		                                       " AND bh.browser = 'DEFAULTB'";

		String query_def_websearch =
		                             "SELECT 'Default Browser Web Search' AS EventType, ws.search_text AS 'Remote Party Contact', ws.search_date AS tldate" +
		                                     " FROM WebSearch ws" +
		                                     " WHERE ws.case_id = " +
		                                     AndrospyMain.gb_CaseId +
		                                     " AND ws.device_id = " +
		                                     AndrospyMain.gb_DeviceId +
		                                     " AND ws.browser = 'DEFAULTB'";

		String query_Def_Bookmark =
		                            "SELECT 'Default Browser Bookmark' AS EventType, b.url AS 'Remote Party Contact', b.modified_date AS tldate" +
		...
		String query_Def_Cookies =
		                           "SELECT 'Default Browser Cookies' AS EventType, CONCAT_WS ('/', bc.host, bc.cookie_name) AS 'Remote Party Contact', bc.last_access_date AS tldate" +
		                                   " FROM Browser_Cookies bc" +
		                                   " WHERE bc.case_id = " +
		                                   AndrospyMain.gb_CaseId +
		                                   " AND bc.device_id = " +
		                                   AndrospyMain.gb_DeviceId +
		                                   " AND bc.browser = 'DEFAULTB'";

		String query_chrome_hist =
		                           "SELECT 'Default Browser Browser History' AS EventType, bh.url AS 'Remote Party Contact', bh.last_visit_date AS tldate" +
		                                   " FROM Browser_History bh" +
		                                   " WHERE bh.case_id = " +
		                                   AndrospyMain.gb_CaseId +
		...
		String query_chrome_Cookies =
		                              "SELECT 'Default Browser Cookies' AS EventType, CONCAT_WS ('/', bc.host, bc.cookie_name) AS 'Remote Party Contact', bc.last_access_date AS tldate" +
		                                      ...
		                                      " AND bc.device_id = " +
		                                      AndrospyMain.gb_DeviceId +
		                                      " AND bc.browser = 'CHROME'";

		String query_credit_card =
		                           "SELECT 'Chrome Credit Card' AS EventType, cc.name AS 'Remote Party Contact', cc.date_modified AS tldate" +
		...
		try {
			...
			if (bBefore && !bAfter) {
				query +=
				         " AND STR_TO_DATE( " + sdatecol + ", '%m/%d/%Y %T') <= STR_TO_DATE('" +
				                 sfromdate + "', '%Y/%m/%d %T')";
			} else if (!bBefore && bAfter) {
				query +=
				         " AND STR_TO_DATE( " + sdatecol + ", '%m/%d/%Y %T') >= STR_TO_DATE('" +
				...
				query +=
				         " AND STR_TO_DATE( " + sdatecol + ", '%m/%d/%Y %T') >= STR_TO_DATE('" +
				                 sfromdate + "', '%Y/%m/%d %T')" + " AND STR_TO_DATE( " + sdatecol +
				                 ", '%m/%d/%Y %T') <= STR_TO_DATE('" + stodate +
				                 "', '%Y/%m/%d %T') ";
			} else if (bBefore && bAfter) {
				query +=
				         " AND (STR_TO_DATE( " + sdatecol + ", '%m/%d/%Y %T') <= STR_TO_DATE('" +
				                 sfromdate + "', '%Y/%m/%d %T')" + " OR STR_TO_DATE(" + sdatecol +
				                 ", '%m/%d/%Y %T') >= STR_TO_DATE('" + stodate +
				                 "', '%Y/%m/%d %T')) ";
			}
