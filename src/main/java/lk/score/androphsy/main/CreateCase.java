...
class DevicePage extends WizardPage {
	...
	protected void CreateCase() {
		...
		InsertCase(con);
		if (bInsert) {
			bInsert = CreateStore(casetemplate.getCase_path(), casetemplate.getCase_id());
		}
	...
	protected void InsertCase(Connection con) {
		...
		String query =
		               "INSERT INTO Androspy_Case (case_name, case_desc, investigator_id, analyst_id)" +
		                       " VALUES ('" +
		                       case_name +
		                       "','" +
		                       case_desc +
	...
	private boolean updateCasePath(String path, int caseId) {
		...
		Connection con = condb.getConnection();
		String query =
		               "UPDATE Androspy_Case SET case_path = '" + path +
		                       "/', case_created = 1 WHERE case_id = " + caseId;
		try {
			Statement stmt = con.createStatement();
			int count = stmt.executeUpdate(query);
			return count == 1;
