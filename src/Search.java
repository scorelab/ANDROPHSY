...
public class Search extends Composite {
	...
	public Search(Composite parent, int style) {
		...
		lblTextSearch.setFont(SWTResourceManager.getFont("Ubuntu", 14,
				SWT.NORMAL));
		lblTextSearch.setLayoutData(new GridData(SWT.CENTER, SWT.FILL, false,
				false, 3, 1));
		...
		GridData gd_txtSearchField = new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1);
	...
	private void search(String type, String srch) {
		...
		try {
			System.out.println("Search  >> " + type + " / " + srch);
			String query = "";
			...
			if (type.equals("keyword")) {
				...
				stacklayout.topControl = tblcmp;
				query = "SELECT 'Phone Book' AS type, contact_no, name, last_time_contacted"
						+ " FROM Contacts WHERE case_id = " + AndrospyMain.gb_CaseId 
						+ " AND device_id = " + AndrospyMain.gb_DeviceId
						+ " AND contact_no like '%" + srch + "%'"
						+ " UNION "
						+ "SELECT CASE "
						...
						+ "ELSE ' ' END AS type,"
						+ " contact_no, name, date FROM Event_Log WHERE case_id = " + AndrospyMain.gb_CaseId 
						+ " AND device_id = " + AndrospyMain.gb_DeviceId
						+ " AND contact_no like '%" + srch + "%'";
			} else if (type.equals("email address")) {
				stacklayout.topControl = tblcmp;
				query = "SELECT 'Gmail' AS type, fromaddress AS 'Remote Part', datesent, datereceive"
				...
				stacklayout.topControl = tblcmp;
				query = "SELECT 'Phone Book' AS Type, name AS 'Remote Party', last_time_contacted AS 'Timestamp'"
						+ " FROM Contacts WHERE case_id = " + AndrospyMain.gb_CaseId 
						+ " AND device_id = " + AndrospyMain.gb_DeviceId
						+ " AND name ";
				if(bcasesensitive){
				...
				query += " like '%" + srch + "%'"
						...
						+ "WHEN type != 1 AND logtype = 300 THEN 'Outgoing SMS Log' "
						+ "ELSE ' ' END AS Type,"
						+ " name AS 'Remote Party', date AS 'Timestamp' FROM Event_Log WHERE case_id = " + AndrospyMain.gb_CaseId 
						+ " AND device_id = " + AndrospyMain.gb_DeviceId
						+ " AND name ";
				if(bcasesensitive){
					query += "COLLATE latin1_general_cs";
				...
				query = "SELECT CASE WHEN browser = 'DEFAULTB' THEN 'Default Bookmark'" 
