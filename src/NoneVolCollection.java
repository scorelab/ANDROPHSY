...
public class NoneVolCollection extends Composite {
	...
	public NoneVolCollection(Composite parent, int style) {
		...
		GridData gd_grpNoneVolatileData = new GridData(SWT.FILL, SWT.FILL,
				false, false, 2, 1);
		...
		grpNoneVolatileData.setText("None Volatile Data Acquisition");
		...
		btnPhysical.setText("Acquire bit-to-bit copy (Physical Acquisition)");
		...
		GridData gd_btnAcquire = new GridData(SWT.LEFT, SWT.CENTER, false,
				false, 1, 1);
		...
		GridData gd_grpBasicDeviceInformation = new GridData(SWT.FILL,
		...
		btnInstalledApplicationList = new Button(grpBasicDeviceInformation,
				SWT.CHECK);
		...
		Button btnReadInformation = new Button(grpBasicDeviceInformation,
				SWT.NONE);
		...
		GridData gd_btnReadInformation = new GridData(SWT.LEFT, SWT.TOP, false,
				false, 1, 1);
		gd_btnReadInformation.heightHint = 28;
		gd_btnReadInformation.widthHint = 138;

		Button btnScreenCapture = new Button(this, SWT.NONE);
		GridData gd_btnScreenCapture = new GridData(SWT.FILL, SWT.FILL, false,
				false, 1, 1);
		...
		btnScreenCapture.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent arg0) {
				File dir = new File(AndrospyMain.gb_CasePath
						+ "Analysis/ScreenCapture");
				if (!dir.exists()) {
					dir.mkdir();
				}
				new AdbExecCmd().captureScreen(AndrospyMain.gb_CasePath
						+ "Analysis/ScreenCapture");
			}
		});
	...
	private void showHashes() {
		...
		try {
			...
			file = new File(AndrospyMain.gb_CasePath
	...
	protected void saveDeviceProperty(DeviceTemplate devicetemplate) {
		try {
			...
			String query = "Update Androspy_Device SET device_name = '"
	...
	private void PhysicalAcquisition(final String partition) {
		...
		try {
				...
				public void run() {
					try {
						...
						if (partition == "SYSTEM") {
							...
							spath = AndrospyMain.gb_CasePath
									+ "Acquire/System.dd";
						...
						BufferedReader buf21 = new BufferedReader(
			...
			buf2 = new BufferedReader(new InputStreamReader(
					proc.getErrorStream()));
			while ((line = buf2.readLine()) != null) {
	...
	protected void LogicalAcquisition(final String partition) {
		...
		SimpleDateFormat sf = new SimpleDateFormat("dd/MM/yyyy HH:mm:ss");
		AndrospyLog.Logdata(sf.format(new Date()), "File system partition acquisition - " + partition);
		Thread t = new Thread(new Runnable() {
